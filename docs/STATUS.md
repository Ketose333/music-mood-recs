...
| 데이터 | MTG-Jamendo 무드/테마 서브셋, `--max-tars 30` 기준으로 통일(README/app.py/노트북/보고서 전부 일치). `scripts/run_download.cmd`는 인자로 TAR 개수를 바꿀 수 있게 파라미터화됨(`run_download.cmd 60` 등, 인자 없으면 기존과 동일하게 30) — **단, 실제로 40/50/60 등으로 재학습 결정 전까지 문서·노트북·모델의 30-TAR 기준은 그대로 유지** |
