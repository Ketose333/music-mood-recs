...
| 담당자 | 본인 (단독 개발) |
| 마지막 업데이트 | 2026-06-25 |
| 목표 릴리즈 또는 마일스톤 | MVP (DL 포트폴리오 데모) |
...
| 항목 | 값 |
| --- | --- |
| 스택 | Python · PyTorch 또는 TensorFlow/Keras(베이스라인 학습 중 확정) · librosa(멜스펙트로그램) · scikit-learn(코사인 유사도) · Streamlit |
| 데이터 | MTG-Jamendo 또는 MagnaTagATune (무드 태그 포함 공개 데이터셋) |
| 모델 | CNN 베이스라인, 필요 시 CRNN으로 확장 |
...
| 다시 검증할 가정 | MTG-Jamendo/MagnaTagATune 중 어느 데이터셋을 최종 채택할지 | 다운로드 용량·라이선스·서브셋 전략 비교 후 확정 필요 |
...
- 무드 태그 포함 공개 데이터셋(MTG-Jamendo 또는 MagnaTagATune) 서브셋 확보.
- 멜스펙트로그램 추출 전처리.
- CNN 베이스라인 학습, 필요 시 CRNN으로 확장.
...
| --- | --- | --- | --- | --- |
| PR-001 | 무드 태그 데이터셋 서브셋을 확보하고 전처리(멜스펙트로그램 추출)한다 | 학습 가능한 형태의 입력 데이터를 준비 | P0 | 데이터 용량 따라 서브셋 전략 결정 |
| PR-002 | CNN 기반 무드 분류 모델을 학습한다 | 곡의 멜스펙트로그램을 입력하면 무드 클래스를 예측 | P0 | 필요 시 CRNN으로 확장 |
| PR-003 | 분류 모델의 임베딩을 추출해 코사인 유사도 기반 추천을 계산한다 | 선택한 곡과 비슷한 무드의 곡 5개를 제시 | P0 | 별도 추천 모델 학습 없음 |
| PR-004 | Streamlit 데모 앱에서 곡 선택 → 무드 예측 → 추천 5곡 흐름을 보여준다 | 평가자가 직접 곡을 선택해 결과를 확인 | P0 | |
| PR-005 | 모델 성능(정확도/F1)과 추천 결과를 평가·기록한다 | 포트폴리오 설명 자료로 사용 | P1 | 추천은 정량 지표가 약해 정성적 사례 비교 위주 |
...
사용자 계정이나 개인 데이터를 다루지 않는다. 공개 라이선스 데이터셋(MTG-Jamendo 또는 MagnaTagATune)만 사용하며, 데이터셋의 라이선스 조건을 README/PRD에 명시한다.
...
- CRNN 이후 Transformer 기반 오디오 모델로 베이스라인 업그레이드.
...
- MTG-Jamendo 오디오 용량이 커서 전체 다운로드가 비현실적 → 서브셋 전략 필요(career `docs/STATUS.md` 메모).
...
- MTG-Jamendo와 MagnaTagATune 중 최종 데이터셋을 어떻게 확정할 것인가(용량·라이선스·무드 태그 체계 비교 필요).
